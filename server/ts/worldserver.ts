...
export default class World {
  ...
  run(mapFilePath) {
    ...
    this.map.ready(function () {
      ...
      _.each(self.map.mobAreas, function (a: any) {
      ...
      _.each(self.map.chestAreas, function (a: any) {
      ...
      // Spawn static chests
      _.each(self.map.staticChests, function (chest: any) {
      ...
      _.each(self.chestAreas, function (area: any) {
  ...
  clearMobHateLinks(mob) {
    var self = this;
    if (mob) {
      _.each(mob.hatelist, function (obj: any) {
    ...
    if (this.zoneGroupsReady) {
      this.map.forEachGroup(function (id) {
        ...
        if (self.groups[id].incoming.length > 0) {
          spawns = <Array<any>>_.each(self.groups[id].incoming, function (
            entity
          ) {
            if (entity instanceof Player) {
  ...
  tryAddingMobToChestArea(mob) {
    _.each(this.chestAreas, function (area: any) {
