...
document.addEventListener(
  ...
  () => {
    ...
    function toggleClass(ele: HTMLElement, name: string) {
    ...
    $barbuttons?.addEventListener("click", (e) => {
      toggleClass(e.target as HTMLElement, "active");
      if (
        e.target === $chatbutton &&
        $chatbutton?.classList.contains("active")
      ) {
        app.showChat();
      } else {
        app.hideChat();
      ...
      if (e.target === $achievementsbutton) {
        app.toggleAchievements();
        if (app.blinkInterval) {
          clearInterval(app.blinkInterval);
        }
        (e.target as HTMLElement).classList.remove("blink");
      }
    });
